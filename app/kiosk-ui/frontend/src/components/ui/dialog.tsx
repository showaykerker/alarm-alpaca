...
>(({ className, children, ...props }, ref) => (
  <DialogPortal>
    ...
    <DialogPrimitive.Content
      ref={ref}
      className={cn(
        // Default size bumped from `max-w-lg / p-6 / gap-4` (2026-05-18,
        // operator request): the 5-inch panel has plenty of pixels and the
        // old size left dialogs feeling cramped under thumb. Sites that
        // pass their own `max-w-*` keep theirs (Doctor / Smoke use 3xl).
...
const DialogFooter = ({ className, ...props }: React.HTMLAttributes<HTMLDivElement>) => (
  <div
    className={cn("flex flex-col-reverse sm:flex-row sm:justify-end sm:space-x-3 gap-3", className)}
    {...props}
  />
);
...
>(({ className, ...props }, ref) => (
  <DialogPrimitive.Title
    ref={ref}
    // Bumped from text-lg (2026-05-18 operator request — 5-inch panel
    // needs all the readability help it can get; dialogs were too small).
